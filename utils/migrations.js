...
const extractIndexesFromTableCreation = function(table_creation) {
  ...
  do {
    ...
    const name = found_index[1].replace(/`/g, '').trim();

    // Removing ` character from columns
    const columns = found_index[2].replace(/`/g, '').trim().split(',');
 ...
 * @returns {string} knex_data
 */
const mapIndexes = function(knex_data, table_creation) {
  ...
  raw_indexes.forEach((index) => {
    ...
    knex_data += fillIndexData(`table.index(${columnsStr}, '${index.name}')`);
  });
...
const generateTableMigration = (table_definition, table_name, table_info) => {
  return new Promise((resolve, reject) => {
    ...
    knex_data = mapIndexes(knex_data, table_info);
